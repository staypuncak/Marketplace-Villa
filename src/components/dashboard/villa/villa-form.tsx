...
export default function VillaForm({ mode, villaId, initialData }: VillaFormProps) {
  ...
  const [selectedFacilities, setSelectedFacilities] = useState<string[]>(initialData?.selectedFacilities ?? [])
  const [customFacilities, setCustomFacilities] = useState<string[]>(initialData?.customFacilities ?? [])
  const [thumbnailPath, setThumbnailPath] = useState(initialData?.thumbnailPath ?? '')
  const [heroPath, setHeroPath] = useState(initialData?.heroPath ?? '')
  const [galleryPaths, setGalleryPaths] = useState<string[]>(initialData?.galleryPaths ?? [])
  ...
  const [checkOutTime, setCheckOutTime] = useState(initialData?.checkOutTime ?? '12:00')
  const [minStay, setMinStay] = useState(initialData?.minStay ?? '')
  const [bookingNotes, setBookingNotes] = useState(initialData?.bookingNotes ?? '')
  ...
  const saveVilla = async () => {
    ...
    try {
      ...
      if (isEdit && villaIdToUse) {
        ...
        await supabase.from('media').delete().eq('villa_id', villaIdToUse)
      } else {
        ...
        villaIdToUse = villa.id
      }

      if (!villaIdToUse) {
        setSaveError('Gagal mendapatkan ID villa.')
        setSaving(false)
        return
      }
      ...
      if (thumbnailPath) {
        mediaInserts.push({ villa_id: villaIdToUse, image_url: thumbnailPath, is_cover: true, sort_order: 0 })
      }
      if (heroPath) {
        mediaInserts.push({ villa_id: villaIdToUse, image_url: heroPath, is_cover: false, sort_order: 1 })
      }
      galleryPaths.forEach((path, i) => {
        mediaInserts.push({ villa_id: villaIdToUse, image_url: path, is_cover: false, sort_order: i + 2 })
      })
      ...
      <div className="mx-auto max-w-lg space-y-8">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-10 text-center shadow-sm">
          ...
          <h2 className="mt-4 text-xl font-semibold text-gray-900">
            {isEdit ? 'Villa berhasil diperbarui' : 'Villa berhasil dibuat'}
          </h2>
          <p className="mt-1 text-sm text-gray-500">
            {isEdit ? 'Perubahan villa sudah tersimpan.' : 'Villa Anda sekarang sudah masuk daftar.'}
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Link
              href={isEdit && villaId ? `/dashboard/villa/${villaId}/edit` : '/dashboard/villa'}
            ...
            >
              {isEdit ? 'Lihat Perubahan' : 'Lihat Villa'}
            </Link>
            {!isEdit && (
              <button onClick={() => { setStep(0); setSuccess(false); router.refresh() }} className="rounded-xl border border-gray-200 px-5 py-2.5 text-sm font-semibold text-gray-700 shadow-sm transition-all hover:bg-gray-50">
                Tambah Villa Lagi
              </button>
            )}
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-semibold text-gray-900">
          {isEdit ? 'Edit Villa' : 'Tambah Villa Baru'}
        </h2>
        <p className="mt-1 text-sm text-gray-500">
          {isEdit
            ? 'Perbarui informasi villa yang sudah ada.'
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          onClick={step < steps.length - 1 ? next : saveVilla}
          disabled={saving}
        ...
        >
          {saving ? (
            ...
            <>{step < steps.length - 1 ? 'Lanjut' : (isEdit ? 'Update Villa' : 'Simpan Villa')} <ArrowRight className="size-4" /></>
          )}
        </button>
      </div>
    </div>
