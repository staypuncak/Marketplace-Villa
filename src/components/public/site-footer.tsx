...
import { BadgeCheck, Tag, MessageCircle } from 'lucide-react'
...
export function SiteFooter({ settings }: { settings: FooterSettings }) {
  const hasSocial = settings.instagram || settings.tiktok || settings.facebook
      ...
      <Container className="py-12 sm:py-16">
        <div className="space-y-8 sm:hidden">
          ...
          <div className="grid grid-cols-2 gap-8">
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-widest text-amber-400/80">
                Navigasi
              </h4>
                ...
                {navLinks.map((link) => (
                  <li key={link.label}>
                    {'action' in link ? (
                      <button
                        onClick={link.action}
                        className="text-sm text-emerald-200/50 transition-colors hover:text-amber-400"
                      >
                        {link.label}
                      </button>
                    ) : (
              ...
              <ul className="mt-4 space-y-3">
                {trustItems.map((item) => (
                  <li key={item.text} className="flex items-center gap-2 text-sm text-emerald-200/50">
                    <item.icon className="size-4 shrink-0 text-amber-400/60" />
                    {item.text}
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-4 flex gap-3">
                {socialLinks.map((s) => (
                  <a
                    ...
                    aria-label={s.label}
                    className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-emerald-200/50 transition-colors hover:bg-amber-400/20 hover:text-amber-400"
                  >
                    <s.icon />
                  </a>
                ))}
        ...
        <div className="hidden gap-10 sm:grid sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="flex gap-2.5">
                {socialLinks.map((s) => (
                  <a
                    ...
                    rel="noopener noreferrer"
                    aria-label={s.label}
                    className="flex h-8 w-8 items-center justify-center rounded-full bg-white/10 text-emerald-200/50 transition-colors hover:bg-amber-400/20 hover:text-amber-400"
                  >
                    <s.icon />
                  </a>
                ))}
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-emerald-200/50">
              <li>WhatsApp: Terhubung</li>
              <li>Email: info@staypuncak.com</li>
              <li>Puncak, Bogor</li>
            </ul>
          </div>
